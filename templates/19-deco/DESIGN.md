...
- 字体栈：`'Limelight', 'Josefin Sans', 'Noto Serif SC', sans-serif`。Limelight 用于英文标题，中文回退 Noto Serif SC。
- **CSS 声明之间不能漏分号**。
- 装饰艺术几何图案用 SVG（扇形、之字形、放射线），必须 `pointer-events: none`。
- 深色背景 `#1A1A2E`，金色 `#D4AF37` + 翡翠绿 `#50C878`。
- 几何对称是核心，装饰元素左右对称。
- 页脚金色文字在深色背景上。
